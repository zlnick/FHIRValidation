...
## 4. Binding strength
    在本IG中，组织机构的language属性的代码绑定强度改为了required，则该字段值域必须符合http://hl7.org/fhir/ValueSet/languages的规约，因此，当该字段取值为wrong language时，因不在required值域中，将导致error级错误
...
## 7. Failing profile
    对于一个资源定义的一个profile包含了多个约束，因此，在校验时所有不满足profile的问题都将被检出，例如本例中包含以下问题：
    1. 错误的language代码
    2. 错误的组织机构类型
    3. 缺少name字段
    可见上述问题都被检出
